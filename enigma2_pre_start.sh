...
wget_file2() {
	download=${1}
	curl ${download} -o $DIR_TMP/$FICHERO
	if [ $? -eq 0 ];
	then
		chmod +x $DIR_TMP/$FICHERO
	else
		echo "Errores al descargar $URL"
		exit 1
	fi
...
limpiar_dir_tmp() {
	...
	then
		...
		borrar_directorio "${DIR_TMP}/astra-arm"
		borrar_directorio "${DIR_TMP}/astra-mips"
		...
		borrar_fichero "${DIR_TMP}/astra-*.zip"
